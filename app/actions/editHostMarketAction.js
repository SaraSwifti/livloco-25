...
async function editHostMarketAction(formData) {
  ...
  const addrPhoneDisplay =
    formData.get('hostfm_address.hostfm_phone_display') || ''
  ...
  if (weekly) {
    ...
    DAYS.forEach((d) => {
      updateData.hostfm_weekly_sched[`${d}_hours`] =
        formData.get(`hostfm_weekly_sched.${d}_hours`) || ''
    })
