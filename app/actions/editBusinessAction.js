...
async function editBusinessAction(formData) {
  ...
  if (hasStorefront) {
    const addrStateCode = (formData.get('locobiz_address.state_code') || '').toString().toUpperCase();
    locobizAddress = {
  ...
  const fm_location_post = formData.get('farmers_market_location.fm_location_post') === 'true';
  ...
  for (const d of days) {
    ...
    farmers_market_location[d] = {
      ...
      zip: formData.get(`farmers_market_location.${d}.zip`) || '',
    };
  }
  ...
  try {
    await LocoBiz.findByIdAndUpdate(existingBiz._id, updateData, { new: true, runValidators: true });
