...
export default async function SavedBusinessesAndMarketsPage() {
  ...
  if (savedBusinessIds.length > 0) {
    ...
    // Fetch both active and inactive businesses that are saved
    const businesses = await LocoBiz.find({
      _id: { $in: savedBusinessIds },
    }).lean()
  ...
  if (savedMarketIds.length > 0) {
    ...
    // Fetch both active and inactive markets that are saved
    const markets = await HostFMarket.find({
      _id: { $in: savedMarketIds },
    }).lean()
