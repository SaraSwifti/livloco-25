...
export default function HostMarketsAddForm({ userEmail }) {
  ...
  // Profile image for Cloudinary
  const [profileUrl, setProfileUrl] = useState('');
  const [uploadedFileName, setUploadedFileName] = useState('');

  // Phones (top-level account market/address phone)
  const [acctPhoneDisplay, setAcctPhoneDisplay] = useState('');
  const [addrPhoneDisplay, setAddrPhoneDisplay] = useState('');

  // Website toggle
  const [showWebsite, setShowWebsite] = useState(false);

  // Stall section toggle
  const [stallAvail, setStallAvail] = useState(false);

  // Schedule mode: 'weekly' | 'random'
  const [scheduleMode, setScheduleMode] = useState('weekly');

  // Random date rows
    const [dates, setDates] = useState([{ id: 1, date: '', time: '' }]);
    const [nextId, setNextId] = useState(2); // ensure each new row gets a fresh id

  const handleDropzoneUpload = async (file) => {
    if (!file) return;
    try {
      const url = await uploadToCloudinary(file); // uses your signed upload flow
      setProfileUrl(url);
  ...
  const handleAddDateRow = () => {
    setDates((prev) => {
   if (prev.length >= MAX_DATES) return prev; // guard
   const row = { id: nextId, date: '', time: '' };
   // bump nextId only when we actually add
   setNextId((n) => n + 1);
   return [...prev, row];
 });
  };
  ...
  const handleRemoveDateRow = (idx) => {
    setDates((prev) => {
      if (prev.length <= 1) return [{ id: 1, date: '', time: '' }]; // keep one blank row
  ...
  const onSubmit = async (e) => {
    ...
    // Inject Cloudinary profile image URL
    form.set('hostfm_profile_image', profileUrl || '');

    // Phones: store E.164 (backend-safe)
    form.set('phone', toE164(acctPhoneDisplay));
    const addrPhone = toE164(addrPhoneDisplay);
    if (addrPhone) form.set('hostfm_address.hostfm_phone', addrPhone);

    // Website normalization (optional)
    const site = normalizeWebsite(form.get('hostfm_website'));
    if (site) {
      form.set('hostfm_website', site);
    } else {
      // allow blank website if invalid/empty
      form.delete('hostfm_website');
    }

    // Host market active on add
    form.set('hostfm_active', 'true');

    // Mutually-exclusive schedule flags
    ...
    // Build random date fields into FormData (if random mode)
    if (!weekly) {
      const capped = dates.slice(0, MAX_DATES); // ✅ hard cap to 6
      capped.forEach((row, idx) => {
        if (row.date) {
          form.set(`hostfm_dates.dates[${idx}].date`, row.date); // yyyy-mm-dd (input[type=date])
          form.set(`hostfm_dates.dates[${idx}].time`, row.time || '');
        }
      });
      // Clean any leftover indices beyond capped length (defensive)
      for (let i = capped.length; i < MAX_DATES + 3; i++) {
        form.delete(`hostfm_dates.dates[${i}].date`);
        form.delete(`hostfm_dates.dates[${i}].time`);
      }
    } else {
      // If weekly, clear any dates[]
      dates.forEach((_, idx) => {
  ...
  {showSkipPopout && (
    <AddBusLaterPopout onClose={handleCloseSkipPopout} />
      ...
      <div className="bg-white p-4 rounded border space-y-4">
        ...
        <div>
          ...
          <input
            ...
            type="text"
            className="border rounded w-full py-2 px-3"
            placeholder="e.g., Produce and Artisan Market"
          />
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded border grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* considering adding this as part of the user profile */}
        ...
        {/* Account phone omitted in this variant */}
        <div>
      ...
      <div className="bg-white p-4 rounded border space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="add_line1" className="block font-bold mb-1">
              Address Line 1
            </label>
                      <input id="add_line1"
          ...
          <div>
            <label htmlFor="add_line2" className="block font-bold mb-1">
              Address Line 2
            </label>
                      <input id="add_line2"
          ...
          <div>
            <label htmlFor="state_code" className="block font-bold mb-1">
              State
            </label>
      ...
      <div className="bg-white p-4 rounded border space-y-3">
        ...
        {showWebsite && (
          <div>
            <label htmlFor="hostfm_website" className="block font-bold mb-1">
              Website URL
            </label>
            <input
              id="hostfm_website"
      ...
      {/* Schedule mode (mutually exclusive) */}
      <div className="bg-white p-4 rounded border space-y-4">
        ...
        {/* Hidden fields that reflect the chosen mode (for schema booleans) */}
        ...
        {scheduleMode === 'weekly' ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {DAYS.map((d) => (
              <div key={d}>
                <label htmlFor={`hostfm_weekly_sched_${d}_hours`} className="block text-sm font-medium capitalize mb-1">
                  {d} hours
                </label>
                <input
