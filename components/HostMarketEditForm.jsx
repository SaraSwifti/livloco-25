...
export default function HostMarketEditForm({ marketData, userEmail }) {
  ...
  const [profileUrl, setProfileUrl] = useState(
    marketData?.hostfm_profile_image || ''
  )
  const [uploadedFileName, setUploadedFileName] = useState('')
  const [deleteProfileImage, setDeleteProfileImage] = useState(false)
  const [isActive, setIsActive] = useState(!!marketData?.hostfm_active)
  ...
  useEffect(() => {
    if (marketData) {
      ...
      if (marketData.hostfm_address?.hostfm_phone) {
        setAddrPhoneDisplay(
          fromE164ToDisplay(marketData.hostfm_address.hostfm_phone)
        )
      }
      ...
      if (hasWeeklySched) {
        ...
        if (
          marketData.hostfm_dates?.dates &&
          Array.isArray(marketData.hostfm_dates.dates)
        ) {
  ...
  const onSubmit = async (e) => {
    ...
    if (site) form.set('hostfm_website', site)
    else form.delete('hostfm_website')

    form.set('hostfm_active', isActive ? 'true' : 'false')
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(
          res.error ||
            'There was a problem updating your market. Please try again.'
        )
      }
    } catch (err) {
      console.error('Submission failed:', err)
      alert(
        'There was a problem updating your market. Please check your inputs and try again.'
      )
    }
  }
    ...
    <>
      ...
      >
        <div className='bg-white p-4 rounded border'>
          <div>
            <h1 className='text-3xl font-bold mb-4'>
              Edit Your Hosted Farmers' Market
            </h1>
            <div className='flex items-center gap-3 mb-6'>
              ...
              />
              <label
                htmlFor='hostfm_active'
                className='font-medium'
              >
                <span className='text-xl font-bold'>Active</span>
                <span className='ml-1 text-base font-normal'>
                  (uncheck to temporarily take down your market. Your data,
                  votes and clicks will still be saved.)
                </span>
              </label>
            </div>
          </div>
          <p className='text-black'>
            LivLoco will never sell or share private account information.
            However, public profile fields will be visible to visitors when your
            post is active.
          </p>
        </div>
        ...
        <div className='bg-white p-4 rounded border grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div>
            <label
              htmlFor='hostfm_address_hostfm_phone'
              className='block font-bold mb-1'
            >
            ...
            <input
              ...
              onChange={(e) =>
                setAddrPhoneDisplay(formatPhoneDisplay(e.target.value))
              }
              className='border rounded w-full py-2 px-3'
            />
          </div>
        ...
        <div className='bg-white p-4 rounded border space-y-4'>
          <h3 className='text-xl font-semibold'>
            Market Address<span className='text-red-500'> required</span>
          </h3>
        ...
        <div className='bg-white p-4 rounded border space-y-4'>
          ...
          <input
            type='hidden'
            name='hostfm_weekly_sched.weekly_sched'
            value={scheduleMode === 'weekly' ? 'true' : 'false'}
          />
          <input
            type='hidden'
            name='hostfm_dates.hostfm_randomdates'
            value={scheduleMode === 'random' ? 'true' : 'false'}
          />
          ...
          {scheduleMode === 'weekly' ? (
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
              {DAYS.map((d) => (
                <div key={d}>
                  <label
                    htmlFor={`hostfm_weekly_sched_${d}_hours`}
                  ...
                  <input
                    ...
                    defaultValue={
                      marketData?.hostfm_weekly_sched?.[`${d}_hours`] || ''
                    }
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      value={row.date}
                      onChange={(e) =>
                        handleDateChange(idx, 'date', e.target.value)
                      }
                      className='border rounded w-full py-2 px-3'
                    />
                  </div>
                  <div className='sm:col-span-2'>
                    ...
                    <input
                      ...
                      value={row.time}
                      onChange={(e) =>
                        handleDateChange(idx, 'time', e.target.value)
                      }
                      className='border rounded w-full py-2 px-3'
                    />
                  </div>
