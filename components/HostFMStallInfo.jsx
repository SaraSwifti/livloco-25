// components/HostFMStallInfo.jsx
'use client';

const toBool = (v) =>
  v === true || (typeof v === 'string' && v.trim().toLowerCase() === 'true') || v === 1;

export default function HostFMStallInfo({ market }) {
  if (!toBool(market?.stall_avail)) return null;

  const { stall_pricing, stall_size, stall_included } = market || {};

  return (
    <section className="w-full">
      <div className="bg-white p-6 rounded-lg ring-1 ring-black/10 shadow-2xl">
        <h2 className="text-2xl font-bold text-black mb-3">Stall Availability</h2>
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <dt className="text-sm text-gray-600">Pricing</dt>
            <dd className="text-gray-900">{stall_pricing || '—'}</dd>
          </div>
          <div>
            <dt className="text-sm text-gray-600">Stall Size</dt>
            <dd className="text-gray-900">{stall_size || '—'}</dd>
          </div>
          <div className="sm:col-span-2">
            <dt className="text-sm text-gray-600">Included</dt>
            <dd className="text-gray-900">{stall_included || '—'}</dd>
          </div>
        </dl>
      </div>
    </section>
  );
}
